...
export const ResponseBox = () => {
  ...
  if (response?.data?.results) {
    const failedResult = response.data.results.find(
      (result: ApiResult) => result.ok === false
    );
    if (failedResult) {
      return (
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          ...
          <div className="space-y-2 text-red-600">
            <p>
              <strong>Mensaje:</strong> {failedResult.message}
            </p>
            <p>
              <strong>Error:</strong> {failedResult.error}
            </p>
          </div>
    ...
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
      <div className="text-gray-500">
        ...
        <p className="font-medium">
          Completa el formulario y envía para ver la respuesta
        </p>
      </div>
    </div>
  );
